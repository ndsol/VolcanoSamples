...
#include "09fullscreen/09snow.frag.h"
#include "09fullscreen/09snow.vert.h"
#include "imgui.h"

namespace example {

#include "09fullscreen/struct_09snow.vert.h"
namespace frag {
#include "09fullscreen/struct_09snow.frag.h"
}

const std::vector<st_09snow_vert> vertices = {
...
// See github.com/ndsol/VolcanoSamples/09fullscreen/ for documentation
class Snow09 : public BaseApplication {
 public:
  static constexpr size_t sampleLayout = 0;
  Snow09(language::Instance& instance, GLFWwindow* window)
    ...
    resizeFramebufListeners.emplace_back(std::make_pair(
        [](void* self, language::Framebuf& framebuf, size_t fbi,
           size_t) -> int {
          return static_cast<Snow09*>(self)->buildFramebuf(framebuf, fbi);
        },
        this));
    uglue.redrawListeners.emplace_back(std::make_pair(
        [](void* self, std::shared_ptr<memory::Flight>& flight) -> int {
          return static_cast<Snow09*>(self)->redraw(flight);
        },
        this));
  ...
  ~Snow09() { screenshotFree(screenshotOpaque); }
 ...
 protected:
  ...
  memory::Image screenshot{cpool.vk.dev};
  void* screenshotOpaque{nullptr};

  int setupSampler(science::Sampler& sampler) {
    screenshotOpaque = screenshotInit(glfwGetPrimaryMonitor(), screenshot);
    if (!screenshotOpaque) {
      logE("setupSampler: screenshotInit failed\n");
      return 1;
    }
    ...
    if (sampler.ctorError()) {
      logE("setupSampler: sampler.ctorError failed\n");
      return 1;
    }
    science::SmartCommandBuffer cmd(uglue.stage.pool, uglue.stage.poolQindex);
    // Transition *zebra.image before buildFramebuf so its layout is correct.
    if (cmd.ctorError() || cmd.autoSubmit() ||
        cmd.barrier(*zebra.image, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL)) {
      logE("zebra transition failed\n");
      return 1;
    }
    return 0;
  }

  void doScreenshot(command::SubmitInfo& submit) {
    science::SmartCommandBuffer cmd(uglue.stage.pool, uglue.stage.poolQindex);
    if (cmd.ctorError()) {
      logE("SmartCommandBuffer failed\n");
      return;
    }
    if (screenshotGrab(screenshotOpaque, screenshot, cmd)) {
      logE("screenshotGrab failed\n");
      return;
    }
    if (science::copyImage1to1(cmd, screenshot, *zebra.image) ||
        cmd.barrier(*zebra.image, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL)) {
      logE("doScreenshot: SmartCommandBuffer failed\n");
      return;
    }
    command::CommandPool::lock_guard_t lock(cpool.lockmutex);
    if (cmd.end() || cmd.enqueue(lock, submit)) {
      logE("doScreenshot: cmd.enqueue failed\n");
      return;
    }
  }
  ...
  int buildPass() {
    ...
    return zebra.setName("zebra") ||
           ...
           zebra.image->setName("zebra.image") || setupSampler(zebra) ||
           ...
           vert->loadSPV(spv_09snow_vert, sizeof(spv_09snow_vert)) ||
           frag->loadSPV(spv_09snow_frag, sizeof(spv_09snow_frag)) ||
  ...
  void updateGUI(bool& wantScreenshot) {
    ImGui::NewFrame();
    ...
    if (ImGui::BeginTabBar("tab_bar_id", ImGuiTabBarFlags_NoTooltip)) {
      if (ImGui::BeginTabItem("window", NULL, 0)) {
        ...
        ImGui::Checkbox("screenshot", &wantScreenshot);
        wantNormal =
        ...
        if (fs.getMonitors().size() < 2) {
          ...
          char text[512];
          for (size_t i = 0; it != monitors.end(); i++, it++) {
  ...
  int redraw(std::shared_ptr<memory::Flight>& flight) {
    bool wantScreenshot = true;
    updateGUI(wantScreenshot);
...
static int addDeviceExtensions(language::Instance& inst) {
  ...
  for (size_t i = 0; i < optional.size(); i++) {
    if (inst.devs.at(0)->isExtensionAvailable(optional.at(i))) {
      inst.devs.at(0)->requiredExtensions.push_back(optional.at(i));
    }
  }
#endif
  return 0;
...
static int createApp(GLFWwindow* window) {
  ...
  if (inst.ctorError(
    ...
    r = std::make_shared<Snow09>(inst, window)->run();
  }
